...
void SmartTeam::attack(Team * enemy) {
    ...
    for (int i = 0; i <this->get_size() ; ++i) {
        ...
        {           Ninja *temp= nullptr;
            ...
            if(this->get_character(i)->getkind()==YNINJA_){temp=dynamic_cast<YoungNinja*>(this->get_character(i));}
                if(this->get_character(i)->getkind()==ONINJA_){temp=dynamic_cast<OldNinja*>(this->get_character(i));}
                    if(this->get_character(i)->getkind()==TNINJA_){temp=dynamic_cast<TrainedNinja*>(this->get_character(i));}
...
void Team::print(){
    cout<<"Team"<<endl;
    for (int i = 0; i < this->get_size(); ++i) {
        cout<<this->get_character(i)->getName()<<"  is alive-"<<this->get_character(i)->isAlive()<<" h_p"<<this->get_character(i)->get_HP()<<endl;
    }
}
